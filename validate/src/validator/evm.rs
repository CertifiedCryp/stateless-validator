...
pub fn replay_block(block: Block, db: &mut CacheDB<WitnessProvider>) -> Result<()> {
    ...
    let initialized_block_env = get_block_env(&block)?;
    ...
    apply_beacon_root_contract_call(
        ...
        block.header.number.ok_or(anyhow!("number is None"))?,
        block.header.parent_beacon_block_root,
        &mut evm,
    )
    .map_err(|e| anyhow!("apply_beacon_root_contract_call failed: {:?}", e))?;
...
fn get_tx_env(tx: &Transaction) -> Result<TxEnv> {
    let signature = tx.signature.ok_or(anyhow!("signature is None"))?;
    let transaction = match tx.transaction_type {
        ...
        Some(2) => RethTransaction::Eip1559(TxEip1559 {
            ...
            max_fee_per_gas: tx
                .max_fee_per_gas
                .ok_or(anyhow!("max_fee_per_gas is None in Eip1559 tx"))?,
            max_priority_fee_per_gas: tx
                .max_priority_fee_per_gas
                .ok_or(anyhow!("max_priority_fee_per_gas is None in Eip1559 tx"))?,
        ...
        Some(3) => RethTransaction::Eip4844(TxEip4844 {
            ...
            max_fee_per_gas: tx
                .max_fee_per_gas
                .ok_or(anyhow!("max_fee_per_gas is None in Eip4844 tx"))?,
            max_priority_fee_per_gas: tx
                .max_priority_fee_per_gas
                .ok_or(anyhow!("max_priority_fee_per_gas is None in Eip4844 tx"))?,
            placeholder: None,
            to: tx.to.ok_or(anyhow!("to is None in Eip4844 tx"))?,
            value: tx.value,
            access_list: tx.access_list.clone().unwrap_or_default(),
            blob_versioned_hashes: tx.blob_versioned_hashes.clone().unwrap_or_default(),
            max_fee_per_blob_gas: tx
                .max_fee_per_blob_gas
                .ok_or(anyhow!("max_fee_per_blob_gas is None in Eip4844 tx"))?,
        ...
        Some(4) => RethTransaction::Eip7702(TxEip7702 {
            ...
            max_fee_per_gas: tx
                .max_fee_per_gas
                .ok_or(anyhow!("max_fee_per_gas is None in Eip7702 tx"))?,
            max_priority_fee_per_gas: tx
                .max_priority_fee_per_gas
                .ok_or(anyhow!("max_priority_fee_per_gas is None in Eip7702 tx"))?,
            to: TxKind::from(tx.to),
            value: tx.value,
            access_list: tx.access_list.clone().unwrap_or_default(),
...
fn get_block_env(block: &Block) -> Result<BlockEnv> {
    ...
    Ok(BlockEnv {
        number: U256::from(header.number.ok_or(anyhow!("number is None"))?),
        ...
        blob_excess_gas_and_price: header
            .excess_blob_gas
            .map(|value| BlobExcessGasAndPrice::new(value as u64, true)),
    })
}
